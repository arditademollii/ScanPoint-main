import { useEffect, useState } from "react";
import api from "../api/axiosInstance";

// ===========================
// TIPET
// ID_Ligjeruesi mbetet me kapitale — toCamel nuk e prek (fillon me 2+ shkronja të mëdha)
// ===========================

interface Ligjerata {
  ID: number;          // Backend kthen "ID" — toCamel e ruan si "ID" (2+ shkronja të mëdha)
  emriLigjerates: string;
  ID_Ligjeruesi: number;
  emriLigjeruesit: string;
}

interface Ligjeruesi {
  ID_Ligjeruesi: number;
  emriLigjeruesit: string;
  departamenti: string;
  email: string;
}

interface FormData {
  emriLigjerates: string;
  ID_Ligjeruesi: number;
}

const emptyForm: FormData = { emriLigjerates: "", ID_Ligjeruesi: 0 };

export default function LigjerataPage() {
  // ===========================
  // STATE
  // ===========================
  const [Ligjeratat, setLigjeratat] = useState<Ligjerata[]>([]);
  const [Ligjeruesit, setLigjeruesit] = useState<Ligjeruesi[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [showForm, setShowForm] = useState(false);
  const [editId, setEditId] = useState<number | null>(null);
  const [formData, setFormData] = useState<FormData>(emptyForm);
  const [submitLoading, setSubmitLoading] = useState(false);

  // 0 = të gjithë nxënësit
  const [filterLigjeruesiId, setFilterLigjeruesiId] = useState<number>(0);

  // ===========================
  // NGARKIM I TË DHËNAVE
  // ===========================
  const fetchLigjeruesit = async () => {
    try {
      const res = await api.get("/api/Ligjeruesi");
      setLigjeruesit(res.data);
    } catch {
      // Nuk e stopojmë faqen nëse dështon Ligjeruesit
    }
  };

  const fetchLigjeratat = async () => {
    setLoading(true);
    setError(null);
    try {
      const url =
        filterLigjeruesiId > 0
          ? `/api/Ligjerata/byLigjeruesi/${filterLigjeruesiId}`
          : "/api/Ligjerata";
      const res = await api.get(url);
      setLigjeratat(res.data);
    } catch (err: any) {
      setError(err.response?.data?.message || "Ngarkimi dështoi");
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchLigjeruesit();
  }, []);

  useEffect(() => {
    fetchLigjeratat();
  }, [filterLigjeruesiId]);

  // ===========================
  // HAP FORMA
  // ===========================
  const handleOpenAdd = () => {
    setEditId(null);
    setFormData(emptyForm);
    setShowForm(true);
  };

  const handleOpenEdit = (n: Ligjerata) => {
    setEditId(n.ID);
    setFormData({
      emriLigjerates: n.emriLigjerates,
      ID_Ligjeruesi: n.ID_Ligjeruesi,
    });
    setShowForm(true);
  };

  // ===========================
  // DORËZIMI I FORMËS
  // ===========================
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!formData.emriLigjerates.trim()) {
      alert("Emri janë të detyrueshme!");
      return;
    }
    if (formData.ID_Ligjeruesi === 0) {
      alert("Duhet të zgjidhni një ligjerues!");
      return;
    }

    setSubmitLoading(true);
    try {
      if (editId !== null) {
        await api.put(`/api/Ligjerata/${editId}`, formData);
      } else {
        await api.post("/api/Ligjerata", formData);
      }
      setShowForm(false);
      fetchLigjeratat();
    } catch (err: any) {
      alert(err.response?.data?.message || "Operacioni dështoi");
    } finally {
      setSubmitLoading(false);
    }
  };

  // ===========================
  // FSHIRJA
  // ===========================
  const handleDelete = async (id: number) => {
    if (!confirm("A je i sigurt?")) return;
    try {
      await api.delete(`/api/Ligjerata/${id}`);
      fetchLigjeratat();
    } catch (err: any) {
      alert(err.response?.data?.message || "Fshirja dështoi");
    }
  };

  // ===========================
  // UI
  // ===========================
  return (
    <div style={{ padding: "20px", maxWidth: "1000px", margin: "0 auto" }}>
      <h1>👨‍🎓 Menaxhimi i ligjeratave</h1>

      <button onClick={handleOpenAdd} style={styles.btnAdd}>
        + Shto ligjerate të Ri
      </button>

      {/* FILTRI */}
      <div style={styles.filterBox}>
        <label style={{ marginRight: "10px", fontWeight: "bold" }}>
          🔍 Filtro sipas ligjeruesit:
        </label>
        <select
          value={filterLigjeruesiId}
          onChange={(e) => setFilterLigjeruesiId(Number(e.target.value))}
          style={styles.select}
        >
          <option value={0}>— Të gjithe Ligjeruesit —</option>
          {Ligjeruesit.map((s) => (
            <option key={s.ID_Ligjeruesi} value={s.ID_Ligjeruesi}>
              {s.emriLigjeruesit} ({s.departamenti})
            </option>
          ))}
        </select>
      </div>

      {/* FORMA */}
      {showForm && (
        <div style={styles.formBox}>
          <h3>{editId ? "✏️ Edito Ligjerates" : "➕ Shto Ligjerate të Ri"}</h3>
          <form onSubmit={handleSubmit}>
            <div style={styles.formGroup}>
              <label>Emri i Ligjerates:</label>
              <input
                type="text"
                value={formData.emriLigjerates}
                onChange={(e) =>
                  setFormData({ ...formData, emriLigjerates: e.target.value })
                }
                placeholder="p.sh. AI"
                style={styles.input}
                required
              />
            </div>


            <div style={styles.formGroup}>
              <label>Ligjeruesi:</label>
              <select
                value={formData.ID_Ligjeruesi}
                onChange={(e) =>
                  setFormData({ ...formData, ID_Ligjeruesi: Number(e.target.value) })
                }
                style={styles.select}
                required
              >
                <option value={0}>— Zgjedh Ligjeruesin —</option>
                {Ligjeruesit.map((s) => (
                  <option key={s.ID_Ligjeruesi} value={s.ID_Ligjeruesi}>
                    {s.emriLigjeruesit} — {s.departamenti}
                  </option>
                ))}
              </select>
            </div>

            <div style={{ display: "flex", gap: "10px" }}>
              <button type="submit" disabled={submitLoading} style={styles.btnSave}>
                {submitLoading ? "Duke ruajtur..." : "💾 Ruaj"}
              </button>
              <button
                type="button"
                onClick={() => setShowForm(false)}
                style={styles.btnCancel}
              >
                ✕ Anulo
              </button>
            </div>
          </form>
        </div>
      )}

      {loading && <p>⏳ Duke ngarkuar...</p>}
      {error && <p style={{ color: "red" }}>❌ {error}</p>}

      {!loading && !error && (
        <>
          <p style={{ color: "#666" }}>
            Gjithsej: <strong>{Ligjeratat.length}</strong> ligjerata
          </p>
          <table style={styles.table}>
            <thead>
              <tr style={styles.tableHeader}>
                <th style={styles.th}>ID</th>
                <th style={styles.th}>Emri i Ligjerates</th>
               
                <th style={styles.th}>Ligjeruesi</th>
                <th style={styles.th}>Veprimet</th>
              </tr>
            </thead>
            <tbody>
              {Ligjeratat.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ textAlign: "center", padding: "20px" }}>
                    Nuk ka ligjerate të regjistruar
                  </td>
                </tr>
              ) : (
                Ligjeratat.map((n) => (
                  <tr key={n.ID} style={styles.tableRow}>
                    <td style={styles.td}>{n.ID}</td>
                    <td style={styles.td}>{n.emriLigjerates}</td>
                    <td style={styles.td}>{n.emriLigjeruesit}</td>
                    <td style={styles.td}>
                      <button onClick={() => handleOpenEdit(n)} style={styles.btnEdit}>
                        ✏️ Edito
                      </button>
                      <button
                        onClick={() => handleDelete(n.ID)}
                        style={styles.btnDelete}
                      >
                        🗑️ Fshi
                      </button>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </>
      )}
    </div>
  );
}

const styles: { [key: string]: React.CSSProperties } = {
  btnAdd: { backgroundColor: "#2ecc71", color: "white", padding: "10px 20px", border: "none", borderRadius: "6px", cursor: "pointer", marginBottom: "16px", fontSize: "16px" },
  filterBox: { backgroundColor: "#eaf4fb", border: "1px solid #aed6f1", borderRadius: "6px", padding: "12px 16px", marginBottom: "20px", display: "flex", alignItems: "center" },
  formBox: { backgroundColor: "#f0f4f8", border: "1px solid #ccc", borderRadius: "8px", padding: "20px", marginBottom: "20px" },
  formGroup: { marginBottom: "12px" },
  input: { display: "block", width: "100%", padding: "8px", marginTop: "4px", border: "1px solid #ccc", borderRadius: "4px", fontSize: "14px" },
  select: { padding: "8px", border: "1px solid #ccc", borderRadius: "4px", fontSize: "14px", minWidth: "250px" },
  btnSave: { backgroundColor: "#3498db", color: "white", padding: "8px 20px", border: "none", borderRadius: "4px", cursor: "pointer" },
  btnCancel: { backgroundColor: "#95a5a6", color: "white", padding: "8px 20px", border: "none", borderRadius: "4px", cursor: "pointer" },
  table: { width: "100%", borderCollapse: "collapse", marginTop: "10px" },
  tableHeader: { backgroundColor: "#2c3e50", color: "white" },
  tableRow: { borderBottom: "1px solid #ddd" },
  th: { padding: "12px 15px", textAlign: "left" },
  td: { padding: "10px 15px" },
  btnEdit: { backgroundColor: "#f39c12", color: "white", padding: "5px 12px", border: "none", borderRadius: "4px", cursor: "pointer", marginRight: "8px" },
  btnDelete: { backgroundColor: "#e74c3c", color: "white", padding: "5px 12px", border: "none", borderRadius: "4px", cursor: "pointer" },
};
