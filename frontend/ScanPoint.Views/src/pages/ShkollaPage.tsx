...
// ===========================
// TIPET (TypeScript interfaces)
// Tregojnë formatin e të dhënave që vijnë nga backend-i
// ===========================
...
// Forma boshe — përdoret kur hapim formën për shtim
const emptyForm: FormData = { emriShkolles: "", qyteti: "" };

export default function ShkollaPage() {
  // ===========================
  // STATE — të dhënat e komponentit
  // useState = kur ndryshon, komponenti ri-renderohet
  // ===========================
  const [shkollat, setShkollat] = useState<Shkolla[]>([]);  // Lista e shkollave
  const [loading, setLoading] = useState(true);              // A po ngarkohet?
  const [error, setError] = useState<string | null>(null);   // Gabim nga API

  const [showForm, setShowForm] = useState(false);           // A shfaqet forma?
  const [editId, setEditId] = useState<number | null>(null); // ID e shkollës për edit
  const [formData, setFormData] = useState<FormData>(emptyForm);
  const [submitLoading, setSubmitLoading] = useState(false); // A po dërgohet forma?

  // ===========================
  // NGARKIM I TË DHËNAVE
  // useEffect = ekzekutohet kur komponenti ngarkohet
  // ===========================
  const fetchShkollat = async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await api.get("/api/Shkolla"); // GET /api/Shkolla
  ...
  useEffect(() => {
    fetchShkollat(); // Ngarko kur komponenti hapet
  }, []); // [] = ekzekuto vetëm një herë

  // ===========================
  // HAP FORMA PËR SHTIM TË RI
  // ===========================
  const handleOpenAdd = () => {
    setEditId(null);           // Jo edit mode
    setFormData(emptyForm);    // Pastro formën
    setShowForm(true);
  };

  // ===========================
  // HAP FORMA PËR EDITIM
  // Mbush formën me të dhënat e shkollës ekzistuese
  // ===========================
  ...
  // ===========================
  // DORËZIMI I FORMËS
  // Nëse editId ka vlerë → PUT (përditëso)
  // Nëse editId është null → POST (krijo të re)
  // ===========================
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId !== null) {
        // PUT — përditëso shkollën ekzistuese
        await api.put(`/api/Shkolla/${editId}`, formData);
      } else {
        // POST — krijo shkollë të re
        await api.post("/api/Shkolla", formData);
      }
      setShowForm(false);
      fetchShkollat(); // Ringarko listën pas ndryshimit
    } catch (err: any) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/api/Shkolla/${id}`); // DELETE /api/Shkolla/{id}
      fetchShkollat(); // Ringarko listën
    } catch (err: any) {
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "900px", margin: "0 auto" }}>
      ...
      {/* Forma e shtimit/editimit — shfaqet vetëm kur showForm=true */}
      {showForm && (
          ...
          <form onSubmit={handleSubmit}>
            <div style={styles.formGroup}>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, emriShkolles: e.target.value })}
              ...
              shkollat.map((s) => (
                <tr key={s.ID_Shkolla} style={styles.tableRow}>
                  ...
                  <td style={styles.td}>
                    ...
                    <button onClick={() => handleDelete(s.ID_Shkolla)} style={styles.btnDelete}>
                      🗑️ Fshi
                    </button>
                  </td>
                </tr>
